    ...
    public class AdminRadar : RustPlugin
    {
        ...
        private class Radar : FacepunchBehaviour
        {
            ...
            private bool ShowActive(DateTime tick)
            {
                ...
                foreach (var target in BasePlayer.activePlayerList.Where(target => target != null && target.transform != null && target.IsConnected))
                {
                    ...
                    if (currDistance < playerDistance)
                    {
                        ...
                        string vanished = ins.Vanish != null && target.IPlayer.HasPermission("vanish.use") && (bool)ins.Vanish.Call("IsInvisible", target) ? "<color=#ff00ffff>V</color>" : string.Empty;
                        string health = showHT && target.metabolism != null ? string.Format("{0} <color=#ffa500ff>{1}</color>:<color=#add8e6ff>{2}</color>", Math.Floor(target.health), target.metabolism.calories.value.ToString("#0"), target.metabolism.hydration.value.ToString("#0")) : Math.Floor(target.health).ToString("#0");
            ...
            private bool ShowSleepers(DateTime tick)
            {
                ...
                foreach (var sleeper in BasePlayer.sleepingPlayerList.Where(target => target != null && target.transform != null))
                {
                    ...
                    if (currDistance < playerDistance)
                    {
                        string health = showHT && sleeper.metabolism != null ? string.Format("{0} <color=#ffa500ff>{1}</color>:<color=#add8e6ff>{2}</color>", Math.Floor(sleeper.health), sleeper.metabolism.calories.value.ToString("#0"), sleeper.metabolism.hydration.value.ToString("#0")) : Math.Floor(sleeper.health).ToString("#0");
            ...
            private bool ShowHeli(DateTime tick)
            {
                ...
                if (cache.Helicopters.Count > 0)
                {
                    foreach (var heli in cache.Helicopters.Where(target => target != null && target.transform != null && !target.IsDestroyed))
                    {
                        ...
                        string info = showHeliRotorHealth ? string.Format("<color={0}>{1}</color> (<color=#ffff00ff>{2}</color>/<color=#ffff00ff>{3}</color>)", healthCC, heliHealth, Math.Floor(heli.weakspots[0].health), Math.Floor(heli.weakspots[1].health)) : string.Format("<color={0}>{1}</color>", healthCC, heliHealth);
                    ...
                    case "drops":
                        {
                            ...
                            foreach (var entity in BaseNetworkable.serverEntities.Where(e => e is DroppedItem || e is Landmine || e is BearTrap || e is DroppedItemContainer))
                            {
                                ...
                                {
                                    if (drawText) player.SendConsoleCommand("ddraw.text", 30f, Color.red, entity.transform.position, string.Format("{0} <color=#ffff00ff>{1}</color>", shortname, currDistance));
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["PreviousFilter"] = "To use your previous filter type <color=#ffa500ff>/{0} f</color>",
                ["Activated"] = "ESP Activated - {0}s refresh - {1}m distance. Use <color=#ffa500ff>/{2} help</color> for help.",
                ...
                ["Help1"] = "<color=#ffa500ff>Available Filters</color>: {0}",
                ["Help2"] = "<color=#ffa500ff>/{0} {1}</color> - Toggles showing online players boxes only when using the <color=#ff0000ff>box</color> filter.",
                ["Help3"] = "<color=#ffa500ff>/{0} {1}</color> - Toggles quick toggle UI on/off",
                ["Help4"] = "<color=#ffa500ff>/{0} {1}</color> - Draw on your screen the movement of nearby players. Must be enabled.",
                ["Help5"] = "e.g: <color=#ffa500ff>/{0} 1 1000 box loot stash</color>",
                ["Help6"] = "e.g: <color=#ffa500ff>/{0} 0.5 400 all</color>",
                ...
                ["Help7"] = "<color=#ffa500ff>/{0} {1}</color> - Toggles showing where players are looking.",
                ["Help8"] = "<color=#ffa500ff>/{0} {1}</color> - Toggles extended information for players.",
                ["backpack"] = "backpack",
                ["scientist"] = "scientist",
                ["npc"] = "npc",
                ["NoDrops"] = "No item drops found within {0}m",
                ["Help9"] = "<color=#ffa500ff>/{0} drops</color> - Show all dropped items within {1}m.",
                ["Zombie"] = "<color=#ff0000ff>Zombie</color>",
